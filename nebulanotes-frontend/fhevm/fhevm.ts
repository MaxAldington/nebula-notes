...
import { isFhevmWindowType, RelayerSDKLoader } from "./internal/loader";
...
export const createFhevmInstance = async (parameters: {
  ...
  if (isMock) {
    ...
    if (fhevmRelayerMetadata) {
      ...
      // isMock=true but metadata is undefined - local Hardhat node is not running or not configured correctly
      throwFhevmError(
        "FHEVM_MOCK_METADATA_ERROR",
        `Cannot fetch FHEVM metadata from ${rpcUrl}. Please ensure:\n1. Hardhat node is running: npx hardhat node\n2. FHEVM is properly configured in hardhat.config.ts\n3. The node is accessible at ${rpcUrl}`
      );
    }
  }
  ...
  // For now, we'll use a simple approach without key storage
  const config: FhevmInstanceConfig = {
    ...relayerSDK.SepoliaConfig,
    network: providerOrUrl,
    publicKey: undefined, // Will be generated
